// Grokking 扫描（论文原格式）：输入 [a, b] 两个 token，输出在最后位置，无「=」符、无 stop-gradient。
// 结构：4 头注意力 + LayerNorm + FFN(4×C) + softmax(P)，full-batch AdamW + WD，梯度已用 gradcheck 验证。
// 本脚本扫描多个权重衰减值，找「顿悟」区间。
...
class Model {
  constructor(P, C=128, nHead=4, scale=0.5){
    this.P=P; this.V=P; this.Vocab=P; this.T=2;   // 论文格式：无「=」符，词表=P，序列 [a,b]
...
function runOne(P, C, LR, WD, maxEpoch, scale=0.5){
  const data=makeData(P,50), n=data.train.length;
  const m=new Model(P,C,4,scale);
  const params=[
  ...
  for(let ep=0; ep<maxEpoch; ep++){
    ...
    if(ep % 100 === 0){
      ...
      if(ep % 1000 === 0 && ep>0){
        ...
        console.log(`    [epoch ${ep}] train=${+(tr*100).toFixed(0)}% test=${+(te*100).toFixed(0)}%`);
      }
    }
  }
  const finalTest=m.evaluate(data.test);
  const secs=((Date.now()-t0)/1000).toFixed(1);
  return { WD, grokEpoch, finalTrain:+(m.evaluate(data.train)*100).toFixed(0), finalTest:+(finalTest*100).toFixed(0), peakTest:+(peakTest*100).toFixed(0), secs };
}

if(require.main===module){
  const P=13, C=128, LR=1e-3, WD=1.0, MAXE=10000, scale=0.1;
  console.log(`=== 论文原格式 [a,b] · P=${P} C=${C} 4头 FFN=4C LayerNorm wd=1.0 scale=0.1 ===`);
  const r=runOne(P,C,LR,WD,MAXE,scale);
  console.log(`P=${P}  grok=${r.grokEpoch===null?'未顿悟':'第'+r.grokEpoch+'步'}  finalTrain=${r.finalTrain}%  finalTest=${r.finalTest}%  peakTest=${r.peakTest}%  耗时${r.secs}s`);
}
